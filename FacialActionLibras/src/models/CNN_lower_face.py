...
Y = np.append(
    np.append(
        y_l_hm[: int(x_l_hm.size / (36 * 98 * 1))],
        y_l_corpus[: int(x_l_corpus.size / (36 * 98 * 1))],
    ),
    y_l_disfa[: int(x_l_disfa.size / (36 * 98 * 1))],
)
##############################################################################
##############################################################################
X = np.nan_to_num(X)
...
model.save_weights(
    "F:\Doutorado\Pesquisa\Python\CNN\saved_cnn_corpus+hm+CK+disfa\cnn_lower_0.h5py"
)

model_json = model.to_json()
with open(
    "F:\Doutorado\Pesquisa\Python\CNN\saved_cnn_corpus+hm+CK+disfa\cnn_lower_0.json",
    "w",
) as json_file:
...
with open(
    "F:\Doutorado\Pesquisa\Python\CNN\saved_cnn_corpus+hm+CK+disfa\cnn_lower.yaml",
    "w",
) as yaml_file:
