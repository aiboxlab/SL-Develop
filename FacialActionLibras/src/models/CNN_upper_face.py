...
##############################################################################
##############################################################################
x_corpus_upper = np.load(
    "F:/Doutorado/Pesquisa/Python/Data_annotations/x_u_corpus.npy"
)
x_hm_upper = np.load(
    "F:/Doutorado/Pesquisa/Python/Data_annotations/x_u_HM.npy"
)
x_disfa_upper = np.load("F:/Doutorado/Pesquisa/Python/cnn_lstm/data/x_u.npy")

y_corpus_upper = np.load(
    "F:/Doutorado/Pesquisa/Python/Data_annotations/y_u_corpus.npy"
)
y_hm_upper = np.load(
    "F:/Doutorado/Pesquisa/Python/Data_annotations/y_u_HM.npy"
)
...
Y = np.append(
    np.append(
        np.append(
            y_corpus_upper[: int(x_corpus_upper.size / (60 * 97 * 1))],
            y_hm_upper[: int(x_hm_upper.size / (60 * 97 * 1))],
        ),
        y_disfa_upper,
    )
)
...
x_train, x_test, y_train, y_test = sk.train_test_split(
    X, Y, train_size=0.7, random_state=seed(2018)
)
...
model.save_weights(
    "E:\Doutorado\Pesquisa\Python\CNN\saved_models_corpus+hm+CK+disfa\cnn_upper_0_60x97.h5py"
)

model_json = model.to_json()
with open(
    "E:\Doutorado\Pesquisa\Python\CNN\saved_models_corpus+hm+CK+disfa\cnn_upper_0_60x97.json",
    "w",
) as json_file:
...
with open(
    "E:\Doutorado\Pesquisa\Python\CNN\saved_models_corpus+hm+CK+disfa\cnn_upper_0_60x97.yaml",
    "w",
) as yaml_file:
...
plt.title(
    "Training and validation accuracy of the model for the expressions on upper part of the face"
)
plt.ylabel("Accuracy")
plt.xlabel("Epochs")
...
plt.title(
    "Training and validation loss of the model for the expressions on upper part of the face"
)
plt.ylabel("Loss")
plt.xlabel("Epochs")
