...
import numpy as np
import pandas as pd
from config.config import CASCADE_PATH, INDICESFACE, PREDICTOR_PATH
...
def read_img_silfa_mid_crop(im_s, location):
    ...
    V_s = data_matrix[:, 1]
    W = np.zeros((int(len(data_matrix)), 1))
        ...
        for i in range(t_inicial.size):
            if i == 0:
                ...
                if t_f[i - 1] == t_i[i]:
                    ...
                    t_i[i] = int(t_inicial[i] * fps)
                    t_f[i] = int(t_final[i] * fps)
        u = np.zeros(int(len(t_i)))
        for i in range(len(t_i)):
            u[i] = t_f[i] - t_i[i]
        w = np.zeros(int(len(t_i) + sum(u)))
        for i in range(len(t_i)):
            if i == 0:
    ...
    for s in range(0, len(V_s)):
        ...
        DT2 = np.zeros((int(len(t_i) + sum(u))))
        DT3 = np.zeros((int(len(t_i) + sum(u))))
        for i in range(len(t_i)):
            if i == 0:
                v0 = t_i[i] + range(0, int(u[i] + 1))
                w[0 : int(len(v0))] = v0
        ...
        for i, j in enumerate(w):
            ...
            if ret is True:
                ...
                zer_0 = np.zeros(28)
                points_lower = np.append(
                    np.hstack(np.vstack(p_l)), zer_0
                ).reshape((50, 2))
                r = cv2.resize(
                ...
                im_u /= 255
                im_l = np.vstack(
                    (lower.T, points_lower[:, 0], points_lower[:, 1])
                )
...
def read_img_DISFA_mid_crop(im_s, name):
    ...
    for s in name:
        ...
        points_l = np.zeros((32, 2))
        data = pd.read_csv(
            ".../DISFA/ActionUnit_Labels/SN0%d.csv" % (s),
            engine="python",
            sep=",",
            skiprows=1,
        )
        ...
        cam = cv2.VideoCapture(
            ".../DISFA/Video_RightCamera/RightVideoSN0%d_Comp.avi" % (s), 0
        )
        Frames = int(cam.get(cv2.CAP_PROP_FRAME_COUNT))
        fps = cam.get(cv2.CAP_PROP_FPS)
        print("Number of Frames", Frames)
...
location_SILFA = (
    "F:/Doutorado/Pesquisa/Python/Data_annotations/corpus FACs Code.csv"
)
# call the function
x_upper, x_lower, y_upper, y_lower = read_img_silfa_mid_crop(
    im_s, location_corpus
)
# save outputs
...
location_HM = (
    "F:/Doutorado/Pesquisa/Python/Data_annotations/HM Database FACs Code.csv"
)
# call the function
x_u_train2, x_l_train2, y_u_train2, y_l_train2 = read_img_HM_mid_crop(
    im_s, location_HM
)
# save outputs
