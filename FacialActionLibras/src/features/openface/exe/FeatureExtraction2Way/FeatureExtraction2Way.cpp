...
int main(int argc, char **argv)
{
	...
	while (true) // this is not a for loop as we might also be reading from a webcam
	{
		...
		while (!captured_image.empty())
		{
			...
			if(visualizer.vis_align || visualizer.vis_hog || visualizer.vis_pose || visualizer.vis_gaze || visualizer.vis_aus || visualizer.vis_lmk){
				if(visualizer.vis_align)
					visualizer.SetObservationFaceAlign(sim_warped_img);
				if(visualizer.vis_hog)
					visualizer.SetObservationHOG(hog_descriptor, num_hog_rows, num_hog_cols);
				if(visualizer.vis_pose)
					visualizer.SetObservationPose(pose_estimate, face_model.detection_certainty);
				if(visualizer.vis_gaze)
					visualizer.SetObservationGaze(gazeDirection0, gazeDirection1, LandmarkDetector::CalculateAllEyeLandmarks(face_model), LandmarkDetector::Calculate3DEyeLandmarks(face_model, sequence_reader.fx, sequence_reader.fy, sequence_reader.cx, sequence_reader.cy), face_model.detection_certainty);
				if(visualizer.vis_aus)
					visualizer.SetObservationActionUnits(face_analyser.GetCurrentAUsReg(), face_analyser.GetCurrentAUsClass());
				if(visualizer.vis_lmk)
			...
			// std::cout << "LANDMARKS" << face_model.detected_landmarks <<std::endl;
			...
			open_face_rec.WriteObservation2Way();
